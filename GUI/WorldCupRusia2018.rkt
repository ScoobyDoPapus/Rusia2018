...
(define (updatePlayers n p t p_old_data p_data ti)
  (cond
    ((equal? t 2) ti)
    ((equal? p 4) (updatePlayers 0 0 (+ t 1) p_old_data p_data ti))
    ((equal? n (length (getPosition p t p_data))) (updatePlayers 0 (+ p 1) t p_old_data p_data ti))
    (else
    (drawPlayer
     ...
     t
     ti)
    (updatePlayers (+ n 1) p t p_old_data p_data ti))
))

;Updates every movement. This function returns the time when the movement of every element is finished
(define (update p_old_data p_data ti tf)
  (sleep (/ 1 60))
  (displayTimeInSeconds 1000 60 ti)
  (cond
    ((< (* ti (/ 1 60)) tf) (update p_old_data p_data (+ (updatePlayers 0 0 0 p_old_data p_data ti) 1) tf))
    (else tf)
  )
)
...
(define (repaint p_old_data p_data ti tf)
  (sleep (/ 1 60))
  (drawCounter 0 0)
  (displayTimeInSeconds 1000 60 ti)
  (cond
    ((< (* ti (/ 1 60)) tf) (repaint p_data (gameData '(1 1 1) '(1 1 1) '(0 0)) (update p_old_data p_data ti tf) tf))
    (else "Game over")
  )
)
...
(define (WCR2018 F1 F2 t)
  (drawField)
  (repaint (gameData F1 F2 '(0 0)) (gameData F1 F2 '(0 0)) 0 t)
)
...
(define (generatePlayer) (list (randInt 795) (randInt 395) (randInt 10) (randInt 10) (randInt 10)))

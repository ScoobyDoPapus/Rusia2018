...
(define (drawBall x1 y1 b_data)
   ((draw-solid-ellipse window) (make-posn (get b_data 0) (get b_data 1)) 10 10 "black")
   (cond
     ((or
       (> (get b_data 0) 795)
       (< (get b_data 0) 0))
      (list (getX x1 (get b_data 0) ))  
     ((or
       (> (get b_data 1) 395)
       (< (get b_data 1) 0))
      (list ())
...
;--------------------------ceeee mamuttt------------------------------------
(define (getFB x1 y1 x2 y2 s d) (cond ((zero? x1 x2) (getY y1 y2 s d)) (else (+ y1 (* (getDirection x1 x2)(* (+ s (getM x1 y1 x2 y2)) d))))))
(define (getYB y1 y2 s d) (+ y1 (* s (getDirection y1 y2) d)))
(define (getXB x1 x2 s d) (+ x1 (* s (getDirection x1 x2) d)))
;--------------------------seeee mamuttt------------------------------------
...
(define (update p_old_data p_data t_abs t)
  ...
  (cond
    ((stop? 0 0 0 p_old_data p_data t) t_abs)
    (else (update p_old_data (replace p_data (drawBall 500 300 (get p_data 2) t_abs) 2) (+ t_abs 1) (+ (updatePlayers 1 0 0 0 p_old_data p_data t_abs t) 1)))
  )
)
...
(define (repaint F1 F2 p_old_data p_data t_abs t gi gf)
  ...
  (displayTimeInSeconds 1000 60 t_abs)
  (cond
    ((< gi gf) (display gi) (newline) (repaint F1 F2 p_data (gameData F1 F2 '(0 50 0 1 1)) (update p_old_data p_data t_abs t) 0 (+ gi 1) gf))
    (else "Game over")
  )
)
;--------------------------------------------------------------------------------------------------------
...
(define (WCR2018 F1 F2 g)
  (drawField)
  (repaint F1 F2 (gameData F1 F2 '(0 50 0 1 1)) (gameData F1 F2 '(0 50 0 1 1)) 0 0 0 g)
)
;Depuration
